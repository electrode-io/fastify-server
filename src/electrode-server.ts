...
async function convertPluginsToArray(plugins) {
    ...
    assert(!topRequireFromPath || _.isString(topRequireFromPath), `config.plugins.requireFromPath must be a string`);
    const loadModule = async (p) => {
        ...
        const getPluginModule = () => {
            ...
            if (_.isString(p.module)) {
                ...
                assert(!p.module.requireFromPath || _.isString(p.module.requireFromPath), `plugin ${p.__name} 'module.requireFromPath' must be a string`);
        ...
        const doRequire = async () => {
            ...
            // 4. mod.default (ES6)
            // 5. mod
            const pluginField = ["fastifyPlugin", "default.fastifyPlugin", "plugin", "default"].find(x => _.get(mod, x));
            // validate plugin
...
export = async function electrodeServer(appConfig = {}, decors) {
    ...
    const makeFastifyServerConfig = context => {
        const fastifyServerConfig = {
            ...
            keepAliveTimeout: _.get(context.config, "keepAliveTimeout", _.get(context.config, "electrode.keepAliveTimeout", DEFAULT_KEEPALIVE_TIMEOUT))
        };
        ...
        //
        (fastifyServerConfig.app as any).config = context.config;
