...
async function convertPluginsToArray(plugins) {
  ...
  const loadModule = async p => {
    ...
    const doRequire = async () => {
      ...
      // 4. mod.default (ES6)
      // 5. mod
      const plugin = ["fastifyPlugin", "default.fastifyPlugin", "plugin", "default"].find(x =>
        _.get(mod, x)
      );
      ...
      const validatePlugin = pos => {
        assert(pos.register, `register of plugin ${name} is a falsy value: ${pos.register}`);
        assert(
          typeof pos.register === "function",
          `register of plugin ${name} should be a function`
        );
      };
      validatePlugin(p);
